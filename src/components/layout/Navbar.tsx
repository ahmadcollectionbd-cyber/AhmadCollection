...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40">
      {/* Main navbar - dark green like Gromuse */}
      <div className="bg-brand-500 dark:bg-slate-900">
        <div className="section flex h-14 items-center gap-4 sm:h-16">
          {/* Logo */}
          <Logo />

          {/* Nav links - desktop */}
          <nav className="ml-2 hidden lg:flex items-center gap-1">
            <NavLink to="/" end className={({ isActive }) => `rounded-lg px-3 py-1.5 text-sm font-medium transition ${isActive ? 'bg-white/20 text-white' : 'text-white/80 hover:text-white hover:bg-white/10'}`}>
              {t('nav.home')}
            </NavLink>
            <NavLink to="/shop" className={({ isActive }) => `rounded-lg px-3 py-1.5 text-sm font-medium transition ${isActive ? 'bg-white/20 text-white' : 'text-white/80 hover:text-white hover:bg-white/10'}`}>
              {t('nav.shop')}
            </NavLink>
            <NavLink to="/track" className={({ isActive }) => `rounded-lg px-3 py-1.5 text-sm font-medium transition ${isActive ? 'bg-white/20 text-white' : 'text-white/80 hover:text-white hover:bg-white/10'}`}>
              {t('nav.track')}
            </NavLink>
            {user?.role === 'admin' && (
              <NavLink to="/admin" className={({ isActive }) => `rounded-lg px-3 py-1.5 text-sm font-medium transition ${isActive ? 'bg-white/20 text-white' : 'text-white/80 hover:text-white hover:bg-white/10'}`}>
                {t('nav.admin')}
              </NavLink>
            )}
          </nav>
          ...
          <div className="ml-auto flex items-center gap-1.5">
            ...
            <div className="relative">
              <button
                ...
                className="relative inline-flex h-9 w-9 items-center justify-center rounded-full text-white/80 transition hover:bg-white/15 hover:text-white"
                aria-label="Notifications"
              >
                <FiBell className="h-4 w-4" />
                {unread > 0 && (
                  <span className="absolute -right-0.5 -top-0.5 inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-orange-500 px-1 text-[10px] font-bold text-white">
                    {unread}
                  </span>
                )}
              </button>
              <AnimatePresence>
                {openNotif && (
                  ...
                  >
                    <div className="border-b border-slate-200/70 px-4 py-2 text-sm font-semibold text-slate-900 dark:border-white/10 dark:text-white">
                      Notifications
                    </div>
            ...
            {user ? (
              <div className="hidden md:flex items-center gap-1.5">
                <Link
                  ...
                  className="inline-flex items-center gap-2 rounded-full bg-white/15 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-white/25"
                >
                  {user.photoURL ? (
                ...
                <button
                  ...
                  className="inline-flex h-9 w-9 items-center justify-center rounded-full text-white/60 hover:bg-white/15 hover:text-white"
                  aria-label="Logout"
                >
              ...
              <Link to="/login" className="hidden md:inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 text-xs font-bold text-brand-700 transition hover:bg-brand-50">
                {t('nav.login')}
              </Link>
            )}
            ...
            <button
              ...
              className="lg:hidden inline-flex h-9 w-9 items-center justify-center rounded-full text-white/80 hover:bg-white/15 hover:text-white"
      ...
      {/* Mobile search bar */}
      <div className="md:hidden border-b border-slate-200/70 bg-white px-4 py-2 dark:border-white/10 dark:bg-slate-950">
        <SearchBar compact />
      </div>
